...
std::wstring s2ws(const std::string& string) {
  ...
  if (size_needed <= 0)
    TZ_THROW(TzError{ "MultiByteToWideChar() failed" });
...
std::string ws2s(const std::wstring& wide_string) {
  ...
  if (size_needed <= 0)
    TZ_THROW(TzError{ "WideCharToMultiByte() failed" });
...
std::tm* gmtime_rs(time_t const* timer, std::tm* buf) {
  ...
  if (res) {
    ...
    error_msg << "failed to call gmtime_rs, with error message "
              << "errno : \"" << res << "\"";
    TZ_THROW(TzError{error_msg.str()});
  }
  return buf;
#else
  std::tm* res = gmtime_r(timer, buf);
  if (!res) [[unlikely]] {
    std::ostringstream error_msg;
    error_msg << "failed to call gmtime_rs, with error message "
...
std::tm* localtime_rs(time_t const* timer, std::tm* buf) {
  ...
  return buf;
#else
  std::tm* res = localtime_r(timer, buf);
  if (!res) [[unlikely]] {
    std::ostringstream error_msg;
    error_msg << "failed to call localtime_rs, with error message "
              << "\"" << strerror(errno) << "\", errno \"" << errno << "\"";
...
void set_thread_name(char const* name) {
#if defined(__CYGWIN__) || defined(__MINGW32__) || defined(__MINGW64__) || defined(TZ_NO_THREAD_NAME_SUPPORT)
  ...
  if (FAILED(hr)) {
    TZ_THROW(TzError{"Failed to set thread name"});
  }
#elif defined(__APPLE__)
  auto const res = pthread_setname_np(name);
  if (res != 0) {
    TZ_THROW(TzError{"Failed to set thread name. error: " + std::to_string(res)});
  }
#else
...
std::string get_thread_name() {
  ...
  if (FAILED(hr)) {
    TZ_THROW(TzError{"Failed to get thread name"});
  }
  ...
  if (res != 0) {
    TZ_THROW(TzError{"Failed to get thread name. error: " + std::to_string(res)});
  }
...
time_t timegm(tm* tm) {
  ...
  if (ret_val == -1) [[unlikely]] {
    TZ_THROW(TzError{"_mkgmtime failed."});
  }
  ...
  if (ret_val == (time_t)-1) [[unlikely]] {
    TZ_THROW(TzError{"timegm failed."});
  }
...
std::vector<char> safe_strftime(char const* format_string, const std::tm* time_info_ptr) {
  ...
  buffer.resize(32);
  // If the length of the resulting C string, including the terminating
  // null-character, doesn't exceed buffer size, the function returns the total
  // number of characters copied to ptr (not including the terminating 
  // null-character).
  // Otherwise, it returns zero, and the contents of the array pointed by ptr
  // are indeterminate.
  size_t res = strftime(&buffer[0], buffer.size(), format_string, time_info_ptr);

  while (res == 0) {
    // if strftime fails we will reserve more space
    buffer.resize(buffer.size() * 2);
    res = strftime(&buffer[0], buffer.size(), format_string, time_info_ptr);
  }
  return buffer;
}
